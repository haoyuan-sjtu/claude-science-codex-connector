...
- 请务必先用 `codex --version` 检查 Codex CLI 版本，必要时更新，并确认 CLI 可以使用 `gpt-5.6-sol` 和 `gpt-5.6-terra`。本连接器不再支持 GPT-5.4。
...
| `codex_model` | 无匹配映射时的默认模型 | `gpt-5.6-sol` |
...
> **重要：** 配置前请先运行 `codex --version` 检查 Codex CLI 版本，必要时升级。当前 CLI
> 必须能够使用 `gpt-5.6-sol` 和 `gpt-5.6-terra`；GPT-5.4 已下线，本连接器不再支持。
> 使用 ChatGPT 账号时，Codex 后端只接受当前 CLI 和账号支持的模型 ID。带后缀的
> `gpt-5.6-sol-codex` 会返回 400 错误。后端同时拒绝 `temperature`、`top_p`、`max_output_tokens`
